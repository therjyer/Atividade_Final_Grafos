...
class MainApp:
    def __init__(self, root):
        ...
        tk.Button(self.root, text="Verificar se o Grafo é Cíclico", command=self.check_if_cyclic).pack(pady=5)
        tk.Button(self.root, text="Gerar uma Ordenação Topológica em um DAG", command=self.check_dag_and_topological_sort).pack(pady=5)
        tk.Button(self.root, text="Verificar se o Grafo é Euleriano", command=self.check_eulerian).pack(pady=5)
        tk.Button(self.root, text="Encontrar Alocação Mínima", command=self.find_minimum_allocation).pack(pady=5)
        tk.Button(self.root, text="Verificar Planaridade", command=self.check_planarity).pack(pady=5)
        tk.Button(self.root, text="Verificar Componentes Fortemente Conectados", command=self.check_strongly_connected_components).pack(pady=5)
