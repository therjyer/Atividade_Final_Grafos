...
class MainApp:
    def __init__(self, root):
        ...
        tk.Button(self.root, text="Encontrar Árvore Geradora Mínima", command=self.find_mst).pack(pady=5)
        tk.Button(self.root, text="Encontrar Caminho de Menor Custo", command=self.find_lowest_cost_path).pack(pady=5)
        tk.Button(self.root, text="Encontrar Caminho Mais Curto", command=self.find_shortest_path).pack(pady=5)
