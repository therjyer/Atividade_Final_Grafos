...
class MainApp:
    def __init__(self, root):
        ...
        tk.Button(self.root, text="Verificar Clique", command=self.check_click).pack(pady=5)
        tk.Button(self.root, text="Verificar Conjunto Dominante", command=self.check_dominating_set).pack(pady=5)
        tk.Button(self.root, text="Verificar Existência de Aresta", command=self.check_edge).pack(pady=5)
        tk.Button(self.root, text="Verificar Conjunto de Vértices Independentes", command=self.check_independent_set).pack(pady=5)
        tk.Button(self.root, text="Verificar Adjacência do Vértice", command=self.check_vertex_adjacency).pack(pady=5)
        tk.Button(self.root, text="Verificar Grau do Vértice", command=self.check_vertex_degree).pack(pady=5)
